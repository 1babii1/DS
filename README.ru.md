...
Next.js (App Router), с аутентификацией, сделанной так, как её реально стоит защищать на
ревью, а не самым быстрым способом заставить кнопку "войти" работать:

- **Auth.js с сессиями на базе данных**, а не JWT в куке — браузер получает только
  непрозрачный, `HttpOnly` идентификатор сессии. OAuth access/refresh токены живут на
  сервере, в отдельной Postgres-схеме `web_auth`, и никогда не попадают в состояние React,
  TanStack Query или `localStorage`.
- **Same-origin BFF-прокси** (`app/api/backend/[...path]/route.ts`) — единственный способ,
  которым браузер достаёт бэкенд: он прикрепляет bearer-токен на сервере, отклоняет
  cross-origin мутации и пропускает только явный allowlist путей бэкенда, а не всё, что
  открывает nginx.
- Департаменты, Позиции, Локации, Люди (найм/перевод) и лента активности на базе
  `AuditService` собраны с настоящими формами создания, пагинацией, фильтрами и доступными
  (accessible) состояниями загрузки/пустоты/ошибки — не просто read-only списки.

## Честный статус

Обе половины проекта проверены вживую на реально запущенном стеке, а не только
юнит-тестами — но они строились и сливались из двух веток, разошедшихся на несколько дней,
и самая новая работа над бэкендом ещё не догнала фронтенд. Сказано прямо, а не сглажено:

- **У RewardsService, NotificationService и SearchService пока нет фронтенда** — не потому,
  что не хватает аутентификации (она есть), а потому что собственный allowlist BFF
  (`app/api/backend/[...path]/route.ts`) пока не включает `/api/rewards`,
  `/api/notifications` или `/api/search`, и ничего их не вызывает. Это реальный следующий
  этап: три работающих бэкенда без единого UI.
- У живого push через SignalR в `NotificationService` пока вообще нет клиента на фронте (нет
  зависимости `@microsoft/signalr`) — REST-лента заработала бы через BFF после добавления в
  allowlist, но real-time push нужна собственная история подключения (BFF-паттерн выше
  устроен под HTTP, а не под WebSocket, и пока не расширен под это).
