...
The People & Organization workspace is a Next.js application for the DS platform. It presents organization data in an accessible, responsive application shell while keeping OAuth credentials on the server.

## Stack

- Next.js App Router and React with TypeScript
- Auth.js with PostgreSQL-backed database sessions
- Tailwind CSS with semantic design tokens, shadcn/ui primitives, and Lucide icons
...
The BFF route only proxies explicitly allowlisted service paths, rejects cross-origin mutations, and never forwards upstream cookies.
