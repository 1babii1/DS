...
export default function DepartmentsPage() {
	...
	return <div className='page'>
		...
		{departments.isPending ? <section aria-busy='true' className='empty-state'><div className='empty-state__icon'><Building2 aria-hidden='true' size={21} /></div><h2>Loading organization structure</h2><p>The directory service is responding with the current department hierarchy.</p></section> : null}
		...
		{departments.data ? <><DepartmentHierarchy departments={departments.data} /><div className='directory-section-heading'><GitBranch aria-hidden='true' size={17} /><span>Department records</span></div><div className='department-grid'>{departments.data.map(department => <DepartmentCard department={department} key={department.id} />)}</div></> : null}
	</div>
}
