...
import { Building2, LogIn, Plus, RefreshCw, Repeat2, SlidersHorizontal, UsersRound, X } from 'lucide-react'
...
export default function PeoplePage() {
	...
	const requiresSignIn = isAxiosError(people.error) && people.error.response?.status === 401
	...
	return <div className='page'>
		...
		{people.isPending ? <section aria-busy='true' className='empty-state'><div className='empty-state__icon'><UsersRound aria-hidden='true' size={21} /></div><h2>Loading people</h2><p>Retrieving the employee directory.</p></section> : null}
		{requiresSignIn ? <section className='empty-state'><div className='empty-state__icon'><LogIn aria-hidden='true' size={21} /></div><h2>Sign in to view people</h2><p>Your workspace session authorizes this protected employee directory.</p><Link className='retry-button' href='/login'><LogIn aria-hidden='true' size={16} />Continue to sign in</Link></section> : null}
		{people.error && !requiresSignIn ? <section className='empty-state' role='alert'><div className='empty-state__icon'><Building2 aria-hidden='true' size={21} /></div><h2>People data is unavailable</h2><p>Check that the employee service is running, then try again.</p><button className='retry-button' onClick={() => people.refetch()} type='button'><RefreshCw aria-hidden='true' size={16} />Retry request</button></section> : null}
		{people.data?.items.length === 0 ? <section className='empty-state'><div className='empty-state__icon'><UsersRound aria-hidden='true' size={21} /></div><h2>No people yet</h2><p>Employee records will appear here after they are added to the platform.</p></section> : null}
