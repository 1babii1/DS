...
function PersonCard({ employee, onGrant, onTransfer }: { employee: Employee; onGrant: (employee: Employee) => void; onTransfer: (employee: Employee) => void }) {
	return <article className='person-card'>
		...
		<div className='person-card__actions'><button className='person-card__action' onClick={() => onTransfer(employee)} type='button'><Repeat2 aria-hidden='true' size={15} />Transfer</button><button className='person-card__action' onClick={() => onGrant(employee)} type='button'>Grant reward</button></div>
	</article>
}
...
export default function PeoplePage() {
	...
	return <div className='page'>
		...
		{rewardEmployee ? <RewardGrantForm employeeId={rewardEmployee.id} employeeName={rewardEmployee.fullName} onClose={() => setRewardEmployee(null)} /> : null}
		...
		{visiblePeople.length ? <section aria-label='People directory' className='people-grid'>{visiblePeople.map(employee => <PersonCard employee={employee} key={employee.id} onGrant={setRewardEmployee} onTransfer={employee => setForm({ employee })} />)}</section> : null}
