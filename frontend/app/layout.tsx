	...
	return (
		<html lang='en' suppressHydrationWarning>
			<body className={`${geistSans.variable} ${geistMono.variable}`}>
				<Providers authenticated={Boolean(session?.user)}><AppShell accountControl={<SessionControl user={session?.user} />} authenticated={Boolean(session?.user)}>{children}</AppShell></Providers>
			</body>
		</html>
	)
}
