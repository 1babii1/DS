...
export function CatalogFilter() {
	...
	return <div className='catalog-filter'>
		...
		<select aria-label='Filter status' onChange={event => { const next = new URLSearchParams(params); next.set('status', event.target.value); router.replace(`${pathname}?${next}`) }} value={active}><option value='active'>Active</option><option value='inactive'>Inactive</option><option value='all'>All statuses</option></select>
