...
function Field({ children, error, label, name }: { children: ReactElement<{ 'aria-describedby'?: string; 'aria-invalid'?: boolean }>; error?: string; label: string; name: string }) { const errorId = `${name}-error`; return <label className='form-field'><span>{label}</span>{cloneElement(children, { 'aria-describedby': error ? errorId : undefined, 'aria-invalid': error ? true : undefined })}{error ? <small id={errorId} role='alert'>{error}</small> : null}</label> }
